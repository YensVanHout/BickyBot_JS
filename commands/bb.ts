...
export default {
  ...
  callback: async ({ args }) => {
    ...
    if (args.includes("addquote") && args.length >= 2) {
      ...
      db.set(quotes.length, quote).then(() => {});
      
      quotes.push(quote);
     
      return "added new quote " + quote + ' at index `' + (quotes.length - 1) + '`.';
    }
    ...
    if(args.includes("deletequote") && args.length >= 2){
      ...
      return db.delete(args[1]).then(() => {
        
        return  `\`${removedQuote}\`` + ' has been deleted from the database.'
